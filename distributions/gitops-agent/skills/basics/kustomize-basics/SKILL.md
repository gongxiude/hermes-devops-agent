...
| Task             | Command                                                                    |
| ---------------- | -------------------------------------------------------------------------- |
| Build manifests  | `kustomize build k8s/prod`                                        |
| Preview changes  | `kubectl diff -k k8s/prod`                                        |
| Apply to cluster | `kubectl apply -k k8s/prod`                                       |
| Validate syntax  | `kustomize build k8s/prod \| kubectl apply --dry-run=client -f -` |
...
```bash
mkdir -p <service_name>/base
cd <service_name>/base
...
```bash
# Preview changes
kubectl diff -k <service_name>/dev

# Apply
kubectl apply -k <service_name>/dev

# Delete
kubectl delete -k <service_name>/dev
```
...
```yaml
# <service_name>/components/monitoring/kustomization.yaml
...
```yaml
# <service_name>/prod/kustomization.yaml
...
#### <service_name>/base/deployment.yaml
...
#### <service_name>/base/service.yaml
...
#### <service_name>/base/serviceaccount.yaml
...
#### <service_name>/dev/kustomization.yaml
...
#### <service_name>/dev/patch-replicas.yaml
...
#### <service_name>/dev/config-values.env
...
#### <service_name>/staging/kustomization.yaml
...
#### <service_name>/staging/patch-replicas.yaml
...
#### <service_name>/staging/patch-tolerations.yaml
...
#### <service_name>/prod/patch-resources.yaml
...
#### <service_name>/prod/patch-affinity.yaml
...
#### <service_name>/prod/patch-security.yaml
...
#### <service_name>/prod/poddisruptionbudget.yaml
...
#### <service_name>/prod/horizontalpodautoscaler.yaml
...
#### <service_name>/prod/networkpolicy.yaml
...
#### <service_name>prod/kustomization.yaml (excerpt)
...
#### <service_name>/prod/json-patch-containers.yaml
...
#### <service_name>/components/monitoring/servicemonitor.yaml
...
#### <service_name>/components/monitoring/prometheusrule.yaml
...
#### <service_name>/components/monitoring/patch-metrics.yaml
...
#### <service_name>/components/ingress/kustomization.yaml
...
#### <service_name>/components/ingress/ingress.yaml
...
#### <service_name>/components/ingress/patch-service.yaml
...
#### <service_name>/components/debug-tools/kustomization.yaml
...
```bash
# Build and validate
kustomize build <service_name>/prod

# Use kubectl (includes additional validation)
kubectl kustomize <service_name>/prod

# Validate against cluster without applying
kubectl apply -k <service_name>/prod --dry-run=server

# Check diff before applying
kubectl diff -k <service_name>/prod
```
...
mkdir -p <service_name>/components/common-settings
...
kustomize build <service_name>/prod | grep "^  name:"

# Check ConfigMap/Secret name with hash
kustomize build <service_name>/prod | grep -A 2 "kind: ConfigMap"
```
...
# Build manifests
kustomize build "<service_name>/${OVERLAY}" > "${OUTPUT_DIR}/all.yaml"
...
kustomize build <service_name>/prod --enable-alpha-plugins --load-restrictor=LoadRestrictionsNone

# Show resources before and after transformation
kustomize build <service_name>/base > base.yaml
kustomize build <service_name>/prod > overlay.yaml
diff base.yaml overlay.yaml

# Validate specific resource
kustomize build <service_name>/prod | kubectl apply --dry-run=client -f -

# Check for YAML syntax errors
kustomize build <service_name>/prod | yamllint -

# Inspect ConfigMap hash generation
kustomize build <service_name>/prod | grep -A 10 "kind: ConfigMap"
```
...
kustomize build --load-restrictor=LoadRestrictionsNone <service_name>/prod

# Build multiple environments in parallel
parallel kustomize build <service_name>/{} ::: dev staging prod
```
