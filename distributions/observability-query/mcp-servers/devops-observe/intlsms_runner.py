...
def resolve_environment(config: dict[str, Any], requested_environment: str | None) -> tuple[str, dict[str, Any]]:
    environments = config.get("environments") or {}
    if not isinstance(environments, dict) or not environments:
        raise ValueError("config.environments must be a non-empty mapping")
    environment = requested_environment or str(config["inspection"]["default_environment"])
    environment_config = environments.get(environment)
    if not isinstance(environment_config, dict):
        raise ValueError(f"unknown environment: {environment}")
    return environment, environment_config


def resolve_env_value(name: str | None) -> str:
    if not name:
        return ""
    return os.environ.get(name, "")


def resolve_kubectl_bin(environment_config: dict[str, Any]) -> str:
    kubernetes = environment_config.get("kubernetes") or {}
    specific_name = kubernetes.get("kubectl_bin_env")
    return resolve_env_value(str(specific_name) if specific_name else None) or os.environ.get("KUBECTL_BIN", "kubectl")


def resolve_kubeconfig(environment_config: dict[str, Any]) -> str:
    kubernetes = environment_config.get("kubernetes") or {}
    specific_name = kubernetes.get("kubeconfig_env")
    return resolve_env_value(str(specific_name) if specific_name else None) or os.environ.get("KUBECONFIG_READONLY") or os.environ.get("KUBECONFIG", "")


def resolve_observability_url(environment_config: dict[str, Any], backend: str) -> tuple[str, str | None]:
    observability = environment_config.get("observability") or {}
    backend_config = observability.get(backend) or {}
    env_name = backend_config.get("endpoint_env")
    return resolve_env_value(str(env_name) if env_name else None), str(env_name) if env_name else None


def kubectl_get_json_for_environment(kind: str, name: str, namespace: str, environment_config: dict[str, Any], timeout: int = 10) -> dict[str, Any]:
    kubectl_bin = resolve_kubectl_bin(environment_config)
    kubeconfig = resolve_kubeconfig(environment_config)
    command = [
        kubectl_bin,
        "get",
        safe_name(kind.lower(), "kind"),
...
def inspect(
    ...
    *,
    ...
    resolved_environment, environment_config = resolve_environment(config, environment)
    namespace = str(environment_config["namespace"])
    cluster = str(environment_config["cluster"])
    path_environment = str(environment_config.get("path_environment", resolved_environment))
    ...
    prometheus_base_url, prometheus_env_name = resolve_observability_url(environment_config, "prometheus")
    loki_base_url, loki_env_name = resolve_observability_url(environment_config, "loki")
    kubeconfig_env_name = str((environment_config.get("kubernetes") or {}).get("kubeconfig_env") or "")
    services = config.get("service_catalog") or []
    for service in services:
        ...
        for query_name, query_spec in config["queries"]["prometheus"].items():
            ...
            if dry_run:
                ...
                status = "unknown"
                summary = f"missing {prometheus_env_name or 'prometheus endpoint env'}"
        ...
        for query_name, query_spec in config["queries"]["loki"].items():
            ...
            if dry_run:
                ...
                status = "unknown"
                summary = f"missing {loki_env_name or 'loki endpoint env'}"
        ...
        if dry_run:
            ...
            try:
                payload = kubectl_get_json_for_environment(str(service["kind"]), str(service["workload"]), namespace, environment_config)
    ...
    audit = {
        ...
        "runtime_selection": {
            "environment": resolved_environment,
            "prometheus_env": prometheus_env_name,
            "loki_env": loki_env_name,
            "kubeconfig_env": kubeconfig_env_name,
        },
