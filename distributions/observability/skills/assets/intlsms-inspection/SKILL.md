...
description: Use when the user asks to inspect international SMS (intlsms / 国际短信) runtime health — e.g. "巡检国际短信", "intlsms 生产巡检", "看下国际短信健康度". This is the business entry that identifies the intlsms target and governance boundary, then routes to the k8s-cluster-inspector workflow.
version: 1.0.0
...
metadata:
  hermes:
    ...
    related_skills: [intlsms-domain-context, k8s-cluster-inspector, skill-policy-gate, audit-trail, secret-redaction]
---
...
**业务识别 + 治理边界声明 + 路由**，不直接调用任何 MCP / 工具——具体巡检由 `k8s-cluster-inspector`
workflow 执行。

```
用户：巡检国际短信
  → intlsms-inspection（本 skill：识别 + 路由）
      → k8s-cluster-inspector（workflow：执行编排）
...
完成识别后，**调用 `k8s-cluster-inspector` workflow**，注入：

- `service_context`（来自 `intlsms-domain-context`，含 Service Baseline）
- `environment`、`inspection_window`、`trigger_id`

由 `k8s-cluster-inspector` 取证据、判级、对账，并经 `audit-trail` / `secret-redaction` 输出。
本 skill 不重复巡检逻辑，也不改写 workflow 的风险等级结论。

## 输出

将 `k8s-cluster-inspector` 的结构化结果原样上报：`risk_level` / `evidence` / `evidence_gaps` /
