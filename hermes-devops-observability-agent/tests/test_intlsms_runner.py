...
def test_live_missing_endpoints_degrades_to_unknown() -> None:
    result = run_cmd(env={"OBSERVE_PROMETHEUS_BASE_URL": "", "OBSERVE_LOKI_BASE_URL": "", "KUBECTL_BIN": "/missing/kubectl"})
    assert result.returncode == 0, result.stderr
    payload = json.loads(result.stdout)
...
""",
    ...
    try:
        ...
        result = run_cmd(
            env={
                "OBSERVE_PROMETHEUS_BASE_URL": base_url,
                "OBSERVE_LOKI_BASE_URL": base_url,
                "KUBECTL_BIN": str(fake_kubectl),
            }
        )
    finally:
            ...
            json.dumps(
                {
                    ...
                    "params": {
                        ...
                        "arguments": {"actor": "pytest", "dry_run": True},
                    },
                }
            ),
            "",
